...
const dict = {
  en: {
    ...
    error: 'Error',
    tools: 'Launch & tools',
    launchDefault: 'Default frontend',
...
for (const language of ['fr', 'ja', 'ru', 'vi']) {
  Object.assign(dict[language], {
    tools: dict.en.tools,
    launchDefault: dict.en.launchDefault,
...
$('launchDefault').addEventListener('click', async () => {
  if (!selectedId) return setStatus(tr('noInstances'))
  await api.openWindow({ instanceId: selectedId, flavor: 'default' })
})

$('launchClassic').addEventListener('click', async () => {
  if (!selectedId) return setStatus(tr('noInstances'))
  await api.openWindow({ instanceId: selectedId, flavor: 'classic' })
})
